...
public class RSInterface extends MethodDefinition{
	public int index;
	...
	public RSWidget getChild(int ind){
		Widget[] childs = methods.game.getClient().widgets()[index];
		...
		if(ind<cacheLength){
			...
			if(temp==null){
				temp = new RSWidget(methods, this, ind);
	...
	public Widget[] getChildrenInternal(){
		Widget[][] interfaces = methods.game.getClient().widgets();
