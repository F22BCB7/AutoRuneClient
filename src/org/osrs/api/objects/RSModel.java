...
public class RSModel extends MethodProvider{
	...
	public RSModel(MethodContext context, Object object){
		super(context);
	...
	public RSModel(MethodContext context, int[] xVertices, int[] yVertices, int[] zVertices, 
	...
	public Point getCenterPoint(RSTile location, int orientation, int heightOffset){
		double localX = location.getX()-methods.game.getMapBaseX();
		double localY = location.getY()-methods.game.getMapBaseY();
	...
	public Point getCenterPoint(int plane, double worldX, double worldY, int orientation, int heightOffset){
		...
		double _x = (worldX)-methods.camera.getX()+vertexX;
		double _z = (worldY)-methods.camera.getY()+vertexZ;
		int _y = methods.calculations.tileHeight(worldX, worldY, plane)-methods.camera.getZ()+vertexY-(heightOffset/2);
		...
		int screenX = ((int) _x * methods.game.getViewportScale()) / (int) _z + (methods.game.getViewportWidth()/2);
		int screenY = (_y * methods.game.getViewportScale()) / (int) _z + (methods.game.getViewportHeight()/2);
	...
	public Point[] projectVertices(int plane, double worldX, double worldY, int orientation, int heightOffset){
		...
		for(int i=0;i<verticesX.length;++i){
			...
			double _x = (worldX)-methods.camera.getX()+vertexX;
			double _z = (worldY)-methods.camera.getY()+vertexZ;
			int _y = methods.calculations.tileHeight((worldX), (worldY), plane)-methods.camera.getZ()+vertexY-heightOffset;
		...
		for (int i = 0; i < numTriangles; i++) {
			try{
				...
				if(point1X<=-1 || point1Y<=-1 ||
						...
						point3X<=-1 || point3Y<=-1)
					continue;
				/*int gameScreenX = methods.game.getViewportWidth();
				int gameScreenY = methods.game.getViewportHeight();
				if(point1X>gameScreenX || point1Y>gameScreenY ||
						point2X>gameScreenX || point2Y>gameScreenY ||
						point3X>gameScreenX || point3Y>gameScreenY)
					continue;*/
				if(!methods.calculations.onViewport(point1X, point1Y) || !methods.calculations.onViewport(point2X, point2Y) ||
						!methods.calculations.onViewport(point3X, point3Y))
					continue;
