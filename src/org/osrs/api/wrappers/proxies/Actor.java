...
public class Actor extends RenderableNode implements org.osrs.api.wrappers.Actor{
	...
	@BMethod(name="addHitUpdate")
	public void _addHitUpdate(int a, int b, int c, int d, int e, int f, int g){	}
	@BDetour
	public void addHitUpdate(int a, int b, int c, int d, int e, int f, int g){
		_addHitUpdate(a, b, c, d, e, f, g);
		if(f==0){//health ratio
			if(this instanceof org.osrs.api.wrappers.Player){
				if(this == Client.localPlayer){
					ScriptDef def = Data.currentScripts.get(Client.clientInstance);
					if(def!=null){
						def.onDeathEvent();
					}
				}
