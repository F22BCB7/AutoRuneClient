...
public class Calculations extends MethodDefinition{
	...
	/**
	 * Calculates the angle to a given tile from the
	 * center tile (local player location).
	 * @param targetTile
	 * @return angle
	 */
	public int angleToTile(RSTile targetTile){
		RSTile me = methods.players.getLocalPlayer().getLocation();
		int angle = (int) Math.toDegrees(Math.atan2(me.getX() - targetTile.getX(), me.getY() - targetTile.getY()));
		angle=((-angle)-90)%360;
	...
	public Point locationToMinimap(RSTile tile) {
		...
		double x = tile.getX() - methods.game.getMapBaseX();
		double y = tile.getY() - methods.game.getMapBaseY();
		int mmX = (int) (x * 4 + 2 - (((center.getX() - methods.game.getMapBaseX())+0.5D)*128) / 32);
		int mmY = (int) (y * 4 + 2 - (((center.getY() - methods.game.getMapBaseY())+0.5D)*128) / 32);
	...
	public Point locationToMinimap(double localX, double localY) {
		...
		if(p==null)return new Point(-1, -1);
		RSTile center = p.getLocation();
		int mmX = (int) (localX * 4 + 2 - (((center.getX() - methods.game.getMapBaseX())+0.5D)*128) / 32);
		int mmY = (int) (localY * 4 + 2 - (((center.getY() - methods.game.getMapBaseY())+0.5D)*128) / 32);
		return worldToMinimap(mmX, mmY);
	}
	/**
	...
	public Point locationToScreen(double tileX, double tileY, double regionOffsetX, double regionOffsetY, int plane, int height) {
		return worldToScreen((tileX - methods.game.getMapBaseX() + regionOffsetX) * 128.0D, (tileY - methods.game.getMapBaseY() + regionOffsetY) * 128.0D, plane, height);
	}
	/**
	...
	public boolean onMap(RSTile tile){
		...
		boolean resize = methods.game.getClient().resizeMode();
	...
	public Point worldToScreen(double X, double Y, int plane, int height) {
		try {
			...
			X -= methods.camera.getX();
			Y -= methods.camera.getY();
			Z -= methods.camera.getZ();
			int curvexsin = Calculations.SIN_TABLE[methods.camera.getYaw()];
			int curvexcos = Calculations.COS_TABLE[methods.camera.getYaw()];
			int curveysin = Calculations.SIN_TABLE[methods.camera.getPitch()];
			int curveycos = Calculations.COS_TABLE[methods.camera.getPitch()];
			...
			if(Y==0)
				return new Point(-1, -1);
			int screenX = ((int) X * methods.game.getViewportScale()) / (int) Y + (methods.game.getViewportWidth()/2);
			int screenY = (Z * methods.game.getViewportScale()) / (int) Y + (methods.game.getViewportHeight()/2);
	...
	public int tileHeight(double x, double y, int plane) {
		int[][][] ground = methods.region.getTileHeights();
		int zidx = plane;
		int x1 = ((int)x) >> 7;
			int y1 = ((int)y) >> 7;
			int x2 = ((int)x) & 0x7F;
			...
			if(zidx<3 && (methods.game.getSceneRenderRules()[1][x1][y1] & 0x2) == 0x2)
				zidx++;
