...
public class Camera extends MethodDefinition{
	...
	public int getX(){
		return methods.game.getClient().cameraX();
	}
	public int getY(){
		return methods.game.getClient().cameraY();
	}
	public int getZ(){
		return methods.game.getClient().cameraZ();
	}
	public int getPitch(){
		return methods.game.getClient().cameraPitch();
	}
	public int getYaw(){
		return methods.game.getClient().cameraYaw();
	}
	public int getAngle(){
	...
	public boolean setAngle(int degrees) {
		...
		if (getAngleTo(degrees) > 5) {
			...
			methods.game.getKeyListener()._keyPressed(event);
			int last=-1;
			for(int i=0;i<20;){
			...
			methods.game.getKeyListener()._keyReleased(event);
