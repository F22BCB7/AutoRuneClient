...
public class PageParser {
	...
	private String getNewBaseLink(){
		...
		//return "http://oldschool82.runescape.com/";
	}
	private boolean parseParams() {
		try {
			System.out.println("Parsing parameters...");
			BASE_LINK=getNewBaseLink();
			System.out.println("Base Link : "+BASE_LINK);
			String HTML = getContent(BASE_LINK);
			if(HTML.equals("")){
				return false;
			}
			...
			parameters.put("4", String.valueOf((new Random().nextInt() << 2 | 1) & 0xffff));//clientID
			parameters.put("9", "ElZAIrq5NpKN6D3mDdihco3oPeYN2KFy2DCquj7JMmECPmLrDP3Bnw");//javascriptSettings
			if(HTML.contains(ARCHIVE_PARAM)){
				jarLocation=BASE_LINK+HTML.substring(HTML.indexOf(ARCHIVE_PARAM)+8, HTML.indexOf(".jar")+4);
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
